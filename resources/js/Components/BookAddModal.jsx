...
export default function BookAddModal({ show, onClose, processing, onAddBook }) {
  ...
  const handleSubmit = e => {
    ...
    if (!form.title || !form.author || !form.price) {
      setErrors({
        title: !form.title ? 'Title is required' : undefined,
        author: !form.author ? 'Author is required' : undefined,
        price: !form.price ? 'Price is required' : undefined,
      });
      return;
    }
    ...
    if (onAddBook) {
      ...
      window.axios.post(route('books.store'), form)
        .then(() => {
          onClose();
          setForm({
            title: '',
